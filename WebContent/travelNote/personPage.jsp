...
<%//显示#自己的#游记卡片界面 这里应该try get  如果userid和自己一样 显示自己，否则显示别人的！！！！修改######
	//可以传入参数 user_id 也可以不传入显示自己的
%>

<%
int user_id=(Integer)request.getSession().getAttribute("user_id");//获取sesson的id
String user_name=(String)request.getSession().getAttribute("user_name"); //用户名

//获取get
String get_id_s = (String)(request.getParameter("user_id"));
int get_id=0;
int isSelf=0;

if(get_id_s == null)//没有获得参数
{
	isSelf=1;
	get_id=user_id;
}
...
{
	get_id=Integer.parseInt(get_id_s); //转int
	if(user_id==get_id ){  //是自己
	isSelf=1;
	get_id=user_id;
}
}
...
e.printStackTrace();
		...
		//XXX的游记
String sql2="select * from user where user_id = "+ get_id ;
ResultSet rs2 = stm.executeQuery(sql2);
String part0 = "<h3>我的游记</h3>";
String part0_1="";
if(rs2.next())
{
	 part0_1="<h3>"+rs2.getString("user_name")+"的游记</h3>";
...
String sql="select * from user_logs where user_id = "+ get_id +" order by log_id";
//数据库添加一条记录sql语句
...
if(isSelf==1)
{
	out.print(part0); // 我的游记
}
else{
	out.print(part0_1); //XXX的游记
}
...
{ 
	...
	String savename ="./LOGs/"+get_id+"_"+log_id+".html"; //文件名
	String filename = request.getRealPath(savename);

	String detailJsp = "logPage.jsp?user_id="+get_id+"&log_id="+log_id; //传到详情jsp的网页
	String authorJsp = "personPage.jsp?user_id="+get_id; //传到用户的游记jsp的网页
	//www.w3cschool.cn/hello?key1=value1&key2=value2 get方法
	

//+ 如果是别人的 输入用户名的游记

String part1=
		"<div class=\"mdl-shadow--4dp \" style=\"float: left;margin-bottom: 5%;border-radius: 5px; width:840px; \">"+
...
<div style="margin-bottom: 10%">
				...
				<div class="cardBox" style="height: 100px">
						  <div class="bodyBox" style="border-right:thin;border-right-style: solid"><a href="textEditor.jsp"><img src="img/填写.png"  alt="填写按钮" class="boxImg"></a>
						<p><a title="写游记" href="textEditor.jsp" style="color: black" >写游记</a></p>
					  </div>
						  <div class="bodyBox"> <img src="img/关注.png" alt="关注按钮" class="boxImg">
						<p><a title="我的关注"  style="color: black" >个人中心</a></p>
					  </div>
				...
				<div class="authorPadding mdl-shadow--2dp mdl-list" style="list-style: decimal">
					...
					<a class="mdl-button mdl-button--colored mdl-js-button mdl-js-ripple-effect mdl-button--accent mdl-list__item" href="logPage.jsp?user_id=1&log_id=1">热点一</a>
					<a class="mdl-button mdl-button--colored mdl-js-button mdl-js-ripple-effect mdl-button--accent mdl-list__item" href="logPage.jsp?user_id=1&log_id=2">热点二</a>
					<a class="mdl-button mdl-button--colored mdl-js-button mdl-js-ripple-effect mdl-button--accent mdl-list__item" href="logPage.jsp?user_id=1&log_id=3">热点三</a>
					<a class="mdl-button mdl-button--colored mdl-js-button mdl-js-ripple-effect mdl-button--accent mdl-list__item" href="logPage.jsp?user_id=1&log_id=4">热点四</a>
				</div>
				</div>
	 				</div>
				  </div>
