...
<%//根据 user_id + log_id ，转到的显示游记详情页面的 地图  #####已完成 ，缺少返回按钮！！%>
 <%
 	...
 	String backJsp="logPage.jsp?user_id="+user_id+"&log_id="+log_id; //返回文章的链接
 %>
 <% String allInfoStr= ""; //从数据库导出的字符串 要传递给js %>
 <% 
 	Connection con = null;
 	Statement stm = null;
 	...
 	String sql="select * from location where log_id = "+ log_id +" and user_id=" + user_id+ " order by location_id";
 	//数据库添加一条记录sql语句
 	ResultSet rs = stm.executeQuery(sql);
 	
	
 	while(rs.next()) //从数据库找到位置信息，传递字符串给前台
 	{ 
 		String lng=""+rs.getDouble("lng");
 		String lat=""+rs.getDouble("lat");
 		String des=rs.getString("des");
 		
 		allInfoStr+=lng+";"+lat+";"+des+";"; //一次为lng维度 lat经度 des描述
...
<!-- new  返回文章详情的按钮 这里最外面不需要引号，js中需要 -->
<a href=
<%=backJsp %>
><input type="button" class="mdl-button mdl-button--raised mdl-js-button mdl-js-ripple-effect mdl-button--accent" style="float: right;margin-top:10px;margin-right: 10%" value="返回文章" >
</a>
...
//图标记模块2 脚本
{
    ...
    var data_str= "<%= allInfoStr%>"; //new获取后台的字符串 问题 是否获取全部？？异步？？
   	...
   	var j=0;
   	for (var i=0; i<=strSet.length-3 ;i+=3 ) //解析字符串到data_info 注意好循环的执行次数！
   	{
   		data_info[j]=([ strSet[i],strSet[i+1],strSet[i+2] ]);
   		j++;
   	}
   	/*
   	for (var i=0;i<j ;i++ ) //解析字符串到data_info
   	{
   		alert(data_info[i])
   	}
   	*/
...
{   
	...
	//	new 根据data_info 标点和画图
	function freshMarkers()	
		{
			map.centerAndZoom(new BMap.Point(data_info[0][0],data_info[0][1]), 15); //设置中心位置
			for(var i=0;i<data_info.length;i++){
